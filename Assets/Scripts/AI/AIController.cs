...
public class AIController : MonoBehaviour
{
    ...
    [Header("Radius")]
    public float detectionRange; //the radius at which AI begins to follow the player
    public float attackRange; //the radius at which AI begins to attack the player
    ...
    void Move()
    {
        ...
        {
            isMove = false; //Stop
            Rotation(true); // Rotation(true - look at player, false look at target)
        }
        else if (Vector2.Distance(transform.position, playerPos.position) < detectionRange) //If the player has entered the pursuit radius
        {
            isAttacked = false;
            isMove = true;
        ...
        else if (isReturnToStartPos && Vector2.Distance(transform.position, playerPos.position) > detectionRange && Vector2.Distance(transform.position, startPosition) > 0) //If isReturnToStartPos is enabled, we return to the starting position
        {
            isMove = true;
            transform.position = Vector2.MoveTowards(transform.position, startPosition, moveSpeed * Time.deltaTime);
            Rotation(false);
        }
        else if (infinitiFollow && isMove)//infinity follow
        {
    ...
    // Rotation(true - look at player, false look at target)
    ...
    void Flip()
    {
        ...
        aiSprite.Rotate(0f, 180f, 0f); // Rotate the sprite by 180 degrees around the y-axis
    }
